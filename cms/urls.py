...
from comment.views import ApprovedCommentView, CommentEditView
...
urlpatterns = [
    ...
    url(r'^authors-list/', PostAuthorContentManagementView.as_view(model=PostAuthor, context_object_name='category'),
        name='authors-list'),
    ...
    url(r'^category-list/', CategoryContentManagementView.as_view(model=Category, context_object_name='category'),
        name='category-list'),
    url(r'^category-create/', ObjectCreateView.as_view(model=Category), name='category-create'),
    url(r'^category-edit/(?P<slug>[\w-]+)/(?P<pk>[0-9]+)/', ObjectEditView.as_view(model=Category),
        name='category-edit'),
    url(r'^category-delete/(?P<pk>[0-9]+)/', ObjectDeleteView.as_view(model=Category), name='category-delete'),

    url(r'^comment-approved/(?P<pk>[0-9]+)/', ApprovedCommentView.as_view(), name='comment-approved'),
    url(r'^comment-edit/(?P<pk>[0-9]+)/', CommentEditView.as_view(), name='comment-edit'),
    url(r'^comment-delete/(?P<pk>[0-9]+)/', ObjectDeleteView.as_view(model=Comment), name='comment-delete'),
